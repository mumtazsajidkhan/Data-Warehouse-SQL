...
BEGIN
	...
	(
	 ...
	 SELECT
			  ...
			  END AS sls_order_dt,
			   ...
			   CASE WHEN sls_ship_dt = 0 OR LEN(sls_ship_dt) != 8 THEN NULL -- Invalid Date Orders
					ELSE CAST(CAST(sls_ship_dt AS VARCHAR) AS DATE)
			   END AS sls_ship_dt,
         
          
		 CASE WHEN sls_due_dt = 0 OR LEN(sls_due_dt) != 8 THEN NULL -- Invalid Date Orders
		 ...
		 END AS sls_due_dt,
			...
			CASE WHEN sls_sales IS NULL OR sls_sales <= 0 OR sls_sales != sls_quantity * ABS(sls_price)
				 ...
				 ELSE sls_sales
			END AS sls_sales, -- Reculculating Sales if original value is missing or incorrect
	...
	REPLACE(cid, '-','') cid, --Invalid Values Replaced
